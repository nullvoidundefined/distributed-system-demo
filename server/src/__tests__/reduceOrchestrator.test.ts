...
const seeding: OrchestratorState = { cycle: 1, paused: false, phase: 'seeding' };
...
describe('reduceOrchestrator', () => {
    ...
    it('scales up when queue depth exceeds threshold and below max nodes', () => {
        const running: OrchestratorState = { cycle: 1, paused: false, phase: 'running' };
    ...
    it('resets into the next cycle', () => {
        const complete: OrchestratorState = { cycle: 1, paused: false, phase: 'complete' };
    ...
    it('a tick while paused produces no effects and no phase change', () => {
        const paused: OrchestratorState = { cycle: 1, paused: true, phase: 'seeding' };
    ...
    it('never emits a crash effect while paused, even when the roll hits', () => {
        const paused: OrchestratorState = { cycle: 1, paused: true, phase: 'running' };
        const ctx = {
            ...baseCtx,
            busyNodeIds: ['node-1', 'node-2'],
            crashRoll: 0,
